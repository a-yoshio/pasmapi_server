...
```bash
passmapi_lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
passmapi_lambda$ sam local start-api
passmapi_lambda$ curl http://localhost:3000/
```
...
```bash
passmapi_lambda$ sam logs -n HelloWorldFunction --stack-name passmapi_lambda --tail
```
...
```bash
passmapi_lambda$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name passmapi_lambda
```
